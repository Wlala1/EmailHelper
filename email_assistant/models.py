...
class Email(Base):
    ...
    graph_message_id: Mapped[str | None] = mapped_column(Text, nullable=True)
    graph_immutable_id: Mapped[str | None] = mapped_column(Text, nullable=True)
    ...
    graph_parent_folder_id: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class ClassifierResult(Base):
    ...
    proposed_category_name: Mapped[str | None] = mapped_column(String(128), nullable=True, index=True)
    created_at_utc: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
